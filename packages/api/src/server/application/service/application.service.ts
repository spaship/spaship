  ...
  async checkDeploymentRecord(propertyIdentifier: string, cluster: string) {
    ...
    if (!deploymentRecord)
      this.exceptionService.badRequestException({
        message: `No environment found for the property: ${propertyIdentifier} on ${cluster} cluster. Please create a new environment selecting ${cluster} as the env type from the SPAship Manager.`
      });
  }
