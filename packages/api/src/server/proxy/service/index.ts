...
export class ProxyService {
  ...
  async createIncident(incident: any) {
    try {
      const baseUrl = await this.getBaseUrl();
      const authToken = await this.getAuthToken();
      const response = await this.httpService.axiosRef.post(`${baseUrl}/incidents`, { ...incident }, {
        headers: { Authorization: authToken },
      });
  ...
  async updateIncident(incidentId: string, updatedData: any) {
    try {
      const baseUrl = await this.getBaseUrl();
      const authToken = await this.getAuthToken();
      const response = await this.httpService.axiosRef.patch(`${baseUrl}/incidents/${incidentId}`, { ...updatedData }, {
        headers: { Authorization: authToken },
      });
