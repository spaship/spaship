...
| **DISABLE AUTH**             | Disable authentication and authorization, granting all visitors full access.  Use only for local development.   | `--auth:disable`              | `SPASHIP_AUTH__DISABLE`               | `auth.disable`              | `false`, except in local dev environments               |
